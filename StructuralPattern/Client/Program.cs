using StructuralPatterns.Domain.PaymentStrategies;
using StructuralPatterns.Interfaces;
using System;
using System.Collections.Generic;

namespace StructuralPatterns.Client
{
     class Program
     {
          static void Main(string[] args)
          {
               IStructuralPatterns _StructuralPatterns = new Domain.Models.StructuralPatterns();

               var menuItems = _StructuralPatterns.GetMenuItems();
               ...
               List<IProduct> orderdProducts = ProcessOrder(_StructuralPatterns);
               double orderPrice = GetOrderListPrice(orderdProducts);
               _StructuralPatterns.GetDelivery(orderdProducts);
               ProcessPayment(orderPrice);
          }

          private static List<IProduct> ProcessOrder(IStructuralPatterns _StructuralPatterns)
          {
               ...
               while (true)
               {
                    ...
                    var orderedItem = _StructuralPatterns.OrderMenuItem(parsedResult);
                    if (orderedItem == null)
                    {
